...
HeatMap.prototype.renderGeneBar = function( i){
  ...
  HeatMap.prototype.rangeColor = function(){
   this.buckets = 5;
   this.colors = colorbrewer.RdYlBu[this.buckets];
   var colorScale = d3.scale.quantile().
   domain([this.parent.data.min, this.buckets , this.parent.maxInData() ])
   .range(this.colors);
   return colorScale;
...
HeatMap.prototype.renderGlobalScale = function(){
  ...
  for (var i = 0; i < this.colors.length; i++) {
    var offset = 100 * i/this.colors.length;
  ...
  rectScale.attr('transform', 'translate(' + offset + ',0)');
  return;
};

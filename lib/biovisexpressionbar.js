...
ExpressionBar.prototype.setupContainer = function(){
  ...
  jQuery( '#' + this.opt.target + '_save_raw_data' ).on('click', function(evt) {
    self.saveRenderedData(self, true);
  });
...
ExpressionBar.prototype.prepareSVGForSaving = function(){
   ...
   if(this.opt.plot == 'Ternary'){
    tPlotSVG = document.getElementById("ternaryPlotSVG");
   }    
  ...
  if (this.opt.plot === 'Ternary'){
    var tPlotMain = serializer.serializeToString(tPlotSVG);
  }
  ...
  tPlotMain = tPlotMain.replace(/^<svg/, '<svg y="' + headHeight + '" x="' + this.opt.labelWidth + '"');
...
ExpressionBar.prototype.saveRenderedData = function(self, saveRawData){
...
for(var i = 0; i < total; i++){
  ...
  for(gene in toSave ){    
    if(saveRawData){
      output += toSave[gene][i].data + "\t" + toSave[gene][i].stdev + "\t";
    } else {

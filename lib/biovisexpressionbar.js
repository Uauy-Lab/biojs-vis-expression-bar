...
ExpressionBar.prototype.updateGroupBy = function(self) { 
  ...
  if(ret){
    sessionStorage.groupBy = self.opt.groupBy;  
  }
...
ExpressionBar.prototype.toggleFactorCheckbox = function(shbtn){
 ...
 // shbtn
 if(selected){
  shbtn.data('selected', false); //TODO dont use the data in the element and use the stored data instead
  this._storeSelectedFactor(shbtn[0].id,false);
  shbtn.removeClass('ui-icon-circle-minus');
  shbtn.addClass('ui-icon-circle-plus');
}else{
  shbtn.data('selected', true);
  this._storeSelectedFactor(shbtn[0].id,true);
...
ExpressionBar.prototype._storeSelectedFactor = function(shbtnID, value){
  sessionStorage.setItem(shbtnID, value);
};


ExpressionBar.prototype._isFactorSelected = function(shbtnID){
  //This is pure "syntatic sugar" and a playground for the sessionStorage
  var val = sessionStorage.getItem(shbtnID);
  return val ; 
};
    ...
    if (groupByValue.constructor === Array) {
      ...
      if(index < 0 ){
        if(shbtn.hasClass('ui-icon-circle-minus')){
          shbtn.removeClass('ui-icon-circle-minus');
          shbtn.addClass('ui-icon-circle-plus');
          self._storeSelectedFactor(name,false);
        }          
      }else{
        if(shbtn.hasClass('ui-icon-circle-plus')){
          shbtn.removeClass('ui-icon-circle-plus');
          shbtn.addClass('ui-icon-circle-minus');
          self._storeSelectedFactor(name,true);
        }
        shbtn.data('selected', true);
      }
    }else{

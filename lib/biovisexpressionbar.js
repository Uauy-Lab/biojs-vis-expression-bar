...
ExpressionBar.prototype.setDefaultOptions = function(){
  this.opt = {
   ...
   highlight: null, 
   groupBy: 'groups', 
   groupBarWidth: ($(window).height() * 0.017),
...
ExpressionBar.prototype.refreshSVG = function() {
  ...
  if(this.opt.showTernaryPlot){
    console.log(`MUST DISPLAY TERNARY PLOT`);
  }
  this.render();
  ...
  d3.json(url, function(error, json) {
    ...
    if(typeof json.tern !== 'undefined'){      
      if(Object.keys(json.tern).length === 3){        
        $(`#${eb.opt.target}_ternSpan`).css('display', 'initial');                
      } else {
        console.log(`This gene doesn't have 3 homoeologues, can't do ternary plot`);
      }
    }
...
ExpressionBar.prototype.render = function() {  
  ...
  if(this.opt.showTernaryPlot){
    console.log(`Wait here for a sec`);
    this.renderObject = new ternaryPlot.TernaryPlot('#plot', plot_opts);
  } else {
    this.renderObject = new barPlot.BarPlot(this);
  }

...
var ExpressionBar = function (options) {
  ...
  console.log('About to pick' + this.opt.plot);
...
ExpressionBar.prototype.saveRenderedData = function(self){
  ...
  var toSave = self.opt.renderedData;
...
ExpressionBar.prototype.maxInData = function(){
  var max = 0;
  for(var i in this.data.renderedData){
    for(var j in this.data.renderedData[i]){
      var curr =this.data.renderedData[i][j]
...
ExpressionBar.prototype.renderTitles = function(){
  ...
  var titleSetOffset = this.data.factors.size  * this.opt.groupBarWidth;

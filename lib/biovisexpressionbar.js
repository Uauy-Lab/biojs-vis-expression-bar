...
ExpressionBar.prototype.setupSVG = function(){
 ...
 this.chartHead = d3.select('#'+this.chartSVGidHead).attr('width', this.opt.width);
 this.chartHead.attr('height', this.opt.headerOffset);
 this.chartFoot = d3.select('#'+this.chartSVGidFoot).attr('width', this.opt.width);
...
ExpressionBar.prototype.setupContainer = function(){
 ...
 this._container.append('<button type="button" id="' +this.opt.target + '_sort">Sort</button>');
 this._container.append('<button type="button" id="' +this.opt.target + '_filter">Filter</button>');
 this._container.append('<div id="' +this.opt.target + '_preferences"></div>');
 this._container.append('<div id="' + this.sortDivId + '"></div>');
 this._container.append('<br/><svg id="' + this.chartSVGidHead + '" ></svg>');
 this._container.append('<br/><svg id="' + this.chartSVGid + '" ></svg>');
 this._container.append('<br/><svg id="' + this.chartSVGidFoot + '" ></svg>');
}
...
ExpressionBar.prototype.renderSortWindow = function(){
  console.log("renderSortWindow");
  var self = this;
  this.renderedOrder = this.data.factorOrder;
  var listText = ''
  var name;
  for(i in this.renderedOrder){
    ...
    for(j in orderedKeys){
      ...
      listText += '<li id="' + this.opt.target + 
      '_sorted_position:' + i + '_' + orderedKeys[j] +
      '"  style="background-color:'+
      bgcolor + '">' + orderedKeys[j] + '</li>'  ;
    }
    listText += '</ul>'
  }
...
////<svg xmlns:xlink="http://www.w3.org/1999/xlink" xmlns="http://www.w3.org/2000/svg" id="bar_expression_viewer_chart_svg" width="900" height="100"><g transform="translate(0,20)"><text x="40" y="15" dx=".35em" height="18">group one(n=2)</text><rect x="0" y="0" height="18" fill="#7fc97f" width="18" opacity="1"/><rect x="20" y="0" height="18" fill="#1b9e77" width="18" opacity="1"/><text x="40" y="55" dx=".35em" height="18">group two(n=2)</text><rect x="0" y="40" height="18" fill="#beaed4" width="18" opacity="1"/><rect x="20" y="40" height="18" fill="#d95f02" width="18" opacity="1"/><text x="40" y="35" dx=".35em" height="18">group tree(n=2)</text><rect x="0" y="20" height="18" fill="#beaed4" width="18" opacity="1"/><rect x="20" y="20" height="18" fill="#1b9e77" width="18" opacity="1"/></g><g><g transform="translate(400,20)"><rect y="0" height="18" fill="#1b9e77" width="2.067423047103962"/><line x1="0" y1="10" x2="4.712745739254702" y2="10" stroke-width="2" stroke="black"/><rect y="40" height="18" fill="#d95f02" width="5.5131281256105655"/><line x1="5.443514370553966" y1="50" x2="5.5827418806671645" y2="50" stroke-width="2" stroke="black"/><rect y="20" height="18" fill="#1b9e77" width="16.342486943774176"/><line x1="8.68497388754835" y1="30" x2="23.999999999999996" y2="30" stroke-width="2" stroke="black"/></g><g transform="translate(650,20)"><rect y="0" height="18" fill="#1b9e77" width="20.67423047103962"/><line x1="0" y1="10" x2="47.12745739254701" y2="10" stroke-width="2" stroke="black"/><rect y="40" height="18" fill="#d95f02" width="55.13128125610565"/><line x1="54.435143705539666" y1="50" x2="55.82741880667164" y2="50" stroke-width="2" stroke="black"/><rect y="20" height="18" fill="#1b9e77" width="163.42486943774176"/><line x1="86.8497388754835" y1="30" x2="240" y2="30" stroke-width="2" stroke="black"/></g></g><rect x="659" y="35.921875" width="76.015625" height="15.265625" visibility="hidden"/><text x="661" y="47" height="18" fill="white" font-size="10" visibility="hidden">count: 4,200</text></svg>
//id="bar_expression_viewer_chart_svg" width="900" height="100"
...
ExpressionBar.prototype._fillGroupByExperiment = function(index, gene, property){
  ...
  var o;
  for(o in g){  
    var newObject = this._prepareGroupedByExperiment(index++,o);
    newObject.gene = gene;
    groups[o] = newObject;
  }
  for(o in e){
...
ExpressionBar.prototype._fillGroupByFactor = function(index, gene, property, groupBy){
 ...
 for(o in g){  
  ...
  if(names.indexOf(description) === -1){
    var newObject = this._prepareGroupedByFactor(index++, description);
    newObject.gene = gene;
  ...
  for(i in arr){
    ...
    this.factors.forEach(function(value, key, map){
      ...
      if(typeof factorValue !== 'undefined'){
        ...
        .on('click', function(){
          console.log("About to sort the data");
          console.log(self.data);
          self.sortRenderedGroups([key], self.data.factorOrder);

...
ExpressionBar.prototype._addSortPriority = function(factor){
...
ExpressionBar.prototype.renderSortWindow = function(){
  ...
  for(i in this.renderedOrder){
    ...
    for(j in orderedKeys){
      ...
      bgcolor + '" data-factor="'+ i +'" data-value="' +orderedKeys[j] +'">' + orderedKeys[j] + '</div>'  ;
    }
    listText += '</div>'
  }
  ...
  for(i in arr){
    ...
    this.factors.forEach(function(value, key, map){
      ...
      if(typeof factorValue !== 'undefined'){
        ...
        .on('click', function(){
          self._addSortPriority(key);
          self.sortRenderedGroups(this.sortOrder, self.renderedOrder);

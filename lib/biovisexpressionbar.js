...
ExpressionBar.prototype.updateGroupBy = function(self) {
  ...
  console.log(selectedCheckbox.val());
  var oldGroupBy = self.opt.groupBy;
  var ret = true;
  if(selectedCheckbox.val()  === 'factors'){
    console.log("Factors");
    console.log(factorOptions);
    var facts = [];
  ...
  console.log("Before");
  console.log(oldGroupBy);
  console.log("new");
  console.log(self.opt.groupBy);
...
ExpressionBar.prototype._prepareSingleObject = function(index, oldObject){
  ...
  newObject.data = []; 
  newObject.data.push(data[0].value); 
  newObject.value = data[0].value;
  newObject.stdev = 0;
  newObject.gene = gene;

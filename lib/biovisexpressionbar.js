...
ExpressionBar.prototype.renderTooltip = function(){
  var barHeight = this.opt.barHeight;
  this.tooltipBox = this.chart.append('foreignObject')
  .attr('width', 200)
  .attr('height', 40)
  .attr('x', 10)
  .attr('visibility', 'hidden')
  .attr('y', 10);
  this.tooltip = this.tooltipBox.append('xhtml:body');
  ...
  .style('font-size', this.opt.barWidth + 'px')
  .style('color', 'white'); 
  this.tooltip.width = 80
...
ExpressionBar.prototype.showTooltip = function(mouseovertext, evt) {
  ...
  var textBox = tooltip.node().getBoundingClientRect();
  var max =  bBox.height -  tooltip.height;

  if(y > max){
    y = svgPosition.top -  50;
  }
  ...
  //if(  bottomBox < bBox.height){
  //  xOffset = textBox.height;
  //  tooltipBox.attr('y', y - xOffset);
  //}

  var padding = 2;
  //this.tooltipBox.attr('x', textBox.left - xOffset - padding);
  //this.tooltipBox.attr('y', textBox.top - padding);

  ...
  d3.json(url, function(error, json) {
    ...
    if(typeof json.tern !== 'undefined'){      
      if(Object.keys(json.tern).length === 3){
        var ternaryPlotOption = `
        <span id="${eb.opt.target}_homSpan">
          <input style="cursor: pointer;" id="${eb.opt.target}_showTernaryPlot" type="checkbox"name="showHomoeologues" value="show">
          <label style="cursor: pointer;" for="${eb.opt.target}_showTernaryPlot">Show Ternary Plot</label>
          </input> 
        </span>`;

        $(ternaryPlotOption).insertAfter('#bar_expression_viewer_homSpan');            
      } else {

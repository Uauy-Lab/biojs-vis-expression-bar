...
ExpressionBar.prototype.setupSVG = function(){
  var self = this;
  var fonstSize = this.opt.fonstSize;
...
ExpressionBar.prototype.setupContainer = function(){
  ...
  <div style="overflow-y: auto;" id="plotContainer"><svg id="${this.chartSVGid}"></svg><div id=${this.ternaryPlotContainer}></div></div>
...
ExpressionBar.prototype.createInitialSessionStorage = function(){
  ...
  d3.json(this.opt.data, function(error, json) {
    ...
    eb._storeValue('selectedFactors', self.data.selectedFactors);
    eb._storeValue('groupBy', eb.opt.groupBy);
    eb.updateGroupBy(eb);
    eb.refresh();
  });
}
  ...
  d3.json(url, function(error, json) {
    ...
    if(typeof json.tern !== 'undefined'){
      if(Object.keys(json.tern).length === 3){
        ...
        $(`#${self.opt.target}_ternSpan`).html("This gene doesn't have have a one-to-one relationship, can't do ternary plot").css('display', 'initial').css('color', 'red');
...
ExpressionBar.prototype.showTooltip = function(mouseovertext, evt) {  
  ...
  if(evt.nodeName != "rect"){
    x = svgPosition.left + 11 + this.opt.labelWidth;
    y = svgPosition.top +  47;
  }
  else{
...
ExpressionBar.prototype.renderChartScale = function(){
  ...
  chartScales.find('#font_size_slidebar').slider({
    min: 17,
    max: 23,
    value: this.opt.fonstSize,
    change: function(event) {
      eb.opt.headerOffset = 0;
      var value = $(this).slider("value");
      eb.opt.fonstSize = value;
      eb.opt.barHeight = value;
      eb.opt.groupBarWidth = value;

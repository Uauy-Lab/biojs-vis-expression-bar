...
var ExpressionBar = function (options) {
  try{
    ...
    console.log(`chartConWidth: ${$(`#${this.chartSVGid}`).width()}
      \nlabelWidth: ${this.opt.labelWidth}
      \nchartSpaceWidth: ${$(`#${this.chartSVGid}`).width() - this.opt.labelWidth}
      \nchartSpaceWidth: ${($(`#${this.chartSVGid}`).width() - this.opt.labelWidth)/$(`#${this.chartSVGid}`).width() * 100}%
      `);
...
ExpressionBar.prototype.setupContainer = function(){
  ...
  var parentWidth = $("#bar_expression_viewer").parent().width() - 50; // 50 is the scrollbar width
		if(parentWidth < 900){
    this.opt.width = 1500;
		} else {
  ...
  <div id="${this.sortDivId}" style="position: relative;" ></div>
  </div>  
  <div style="overflow-y: scroll; height: inherit;" id="${this.plotContainer}">

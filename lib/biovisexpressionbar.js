...
ExpressionBar.prototype.sortRenderedGroups = function(sortOrder){
 ...
 for (var i = 0; i < sortable.length; i++) {
   sortable[i].renderIndex = i;
 };

 for(var i = 1; i < this.renderedData.length; i++){
  for (var j = 0; j < sortable.length; j++) {

    this.renderedData[i][sortable[j].id].renderIndex = sortable[j].renderIndex;
  };
 }
...
ExpressionBar.prototype.refreshTitles = function(){
  ...
  .attr("y", function(d,i){
    ...
    console.log("Refresnihg titles: " + groupIndex);
    console.log( arr[groupIndex].renderIndex); 
    console.log(arr[groupIndex]);

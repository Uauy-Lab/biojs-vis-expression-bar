...
var ExpressionBar = function (options) {
 ...
 this._container.append('<button id="' + this.opt.target + '-merge-button">Merge by</button>');
...
ExpressionBar.prototype.renderMergeButtonn = function() {
  var self = this;
  ...
  <form>\
      ...
      <input type="radio" name="group" value="groups"> Experiment<br/>\
...
this._container.append(formText + checkboxes + formTextEnd);
this.joinForm = jQuery('#'+ this.opt.target+'-join-form');
this.joinDialog = this.joinForm.dialog({
      autoOpen: false,
      height: 600,
      width: 500,
      modal: true,
...
jQuery( '#' + this.opt.target + '-merge-button').button().on( "click", function() {
      self.joinDialog.dialog( "open" );
});
...
ExpressionBar.prototype.refreshBar = function(gene, i){
  ...
  var lines = bar.selectAll('line').transition().duration(1000)
     // .attr('x1', gXOffset)
...
ExpressionBar.prototype.renderGeneBar = function( i){
  ...
  for(var j in  dat){
    var d = dat[j];
    var y = (barHeight * d.renderIndex  ) + this.opt.headerOffset;
    bar.append('rect')
    .attr('y', y)
    ...
    .on('mouseenter', function(da,k){
      ...
      //didn't work.  
      var index = ((pos.top - self.opt.headerOffset) / self.opt.barHeight)-1;

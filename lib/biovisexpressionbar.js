...
ExpressionBar.prototype.setupContainer = function(){
  ...
  var parentWidth = $("#bar_expression_viewer").parent().width() - 50;
		if(parentWidth < 900){
    this.opt.width = 1500;
		} else {
  ...
  this.ternaryPlotContainer = this.opt.target + '_ternary_plot_container';
  ...
  <input type="checkbox" id="${this.opt.target}_log2" style="cursor: pointer;"><label for="${this.opt.target}_log2" style="cursor: pointer;">log2</label></input>
  ...
  <span id="${this.opt.target}_ternSpan" style="display: none;"><input style="cursor: pointer;" id="${this.opt.target}_showTernaryPlot" type="checkbox"name="showHomoeologues" value="show"><label style="cursor: pointer;" for="${this.opt.target}_showTernaryPlot">Show Ternary Plot</label></input></span>
  ...
  <div style="overflow-y: scroll; height: inherit;" id="plotContainer"><svg id="${this.chartSVGid}"></svg></div>
  ...
  $(`#${this.opt.target}_showTernaryPlot`)
    .on('change', function(evt){
      ...
      // self.opt.showTernaryPlot = !(self.opt.showTernaryPlot);      
...
ExpressionBar.prototype.setDefaultOptions = function(){
  this.opt = {
   ...
   width: $('#bar_expression_viewer').parent().width() - 500,
   height: ($(window).height() * 0.6),
   barHeight: 17,
   labelWidth: ($('#bar_expression_viewer').parent().width() * 0.4),
   renderProperty: 'tpm',
   renderGroup: 'group',
...
ExpressionBar.prototype.refreshSVG = function() {
  ...
  var plot=d3.select(`#${this.ternaryPlotContainer}`);
...
ExpressionBar.prototype.renderSortWindow = function(){
  ...
  for(var fo in this.data.defaultFactorOrder){
    ...
    factorDiv.css('margin-left',this.opt.groupBarWidth-iconWidth);
...
ExpressionBar.prototype.saveFactorsInOutput = function(selectedFactors, factorNames, output){
...
ExpressionBar.prototype.saveRenderedData = function(self){
 // console.log("saveRenderedData");
  ...
  var factorNames = this.data.longFactorName;
 	...
 	output = this.saveFactorsInOutput(selectedFactors, factorNames, output);
...
ExpressionBar.prototype.saveRawData = function(self){
		...
		var selectedFactors = self.data.selectedFactors;
		var allExperiments = allData.experiments;
		var output = '';
		var factorNames = this.data.longFactorName;
		var renderedProperty = self.opt.renderProperty ;
		var valueObject = allData.values;
		var filename = renderedProperty +  "_raw_data.tsv";
		...
		output += "\nExperiments\tFactors\tValue\\" + renderedProperty + "\n";
		...
		// TODO Add factors
		// Printing the experiments, factors and values
		for(var experiment in allExperiments){
			var group = allExperiments[experiment].group;
			output += "\n" + allExperiments[experiment].name + "\t";
			for(var gene in valueObject){
				var value = 0;
				// There is a 61 gap between group 240 and 302 (puts 0 instead)
				if(typeof valueObject[gene][renderedProperty][group] !== 'undefined'){
					value = valueObject[gene][renderedProperty][group].value;
				}
				output += "\t" + value;
			}
		}
...
ExpressionBar.prototype.loadExpression = function(url) {
  ...
  d3.json(url, function(error, json) {
    ...
    if(typeof json.tern !== 'undefined'){
      if(Object.keys(json.tern).length === 3){
        $(`#${self.opt.target}_ternSpan`).css('display', 'initial');
      } else {
        $(`#${self.opt.target}_ternSpan`).html("No homologies for ternary plot").css('display', 'initial').css('color', 'red');
        console.log(`This gene doesn't have have a one-to-one relationship, can't do ternary plot`);
      }
    }
...
ExpressionBar.prototype.renderGeneTitles = function(i){
  ...
  //   newHeigth =  0.75 * this.opt.barHeight * (this.opt.headerOffset/renderedTextWidth);
  // }
...
ExpressionBar.prototype.showTooltip = function(mouseovertext, evt) {  
  ...
  this.tooltipBox.attr('transform',`translate(${0}, ${0})`);
  tooltip.attr('visibility', 'visible');
  this.tooltip.attr('transform',`translate(${0}, ${0})`);
...
ExpressionBar.prototype.render = function() {
  ...
  if (this.opt.plot === 'Ternary' && this.totalHeight < 700){
...
ExpressionBar.prototype.renderChartScale = function(){
  ...
  chartScales.find('#font_size_slidebar').slider({
    ...
    change: function(event) {
      ...
      self.opt.groupBarWidth = value;

      // Icons resizing
      var iconWidth = $('.bar_expression_viewer_factor').width();
      $('.bar_expression_viewer_factor').css('margin-left',(value-iconWidth)-0.5);
...
ExpressionBar.prototype.resizeChart =  function(){
  ...
  if($(window).width() < 900){
    ...
    parentWidth = $("#bar_expression_viewer").parent().width();
  }

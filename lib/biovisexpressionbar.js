...
var ExpressionBar = function (options) {
  ...
  this.createInitialSessionStorage();
};
...
ExpressionBar.prototype.createInitialSessionStorage = function(){  
  var toSearch='#bar_expression_viewer_sort_div input:checkbox';
  // self = this;
  this.refreshSVGEnabled = false;
  console.log(`DATA: ${this.opt.data}`);
  ...
  console.log(`DATA: ${this.opt.data}`);
  this._storeValue('selectedFactors', this.opt.data.selectedFactors);
...
ExpressionBar.prototype.renderSortWindow = function(){
  ...
  for(fo in this.data.defaultFactorOrder){
    ...
    for(j in orderedKeys){
      ...
      // console.log(`Here is the short id: ${shortId}`);******************aaaaaaaaaaaaaaaaaahhhhhhhhhhhhhhhhhhhhhhhhhh
      var checked = '';
  ...
  for(var fo in this.data.defaultFactorOrder){
    i = this.data.defaultFactorOrder[fo]
    var name=this.opt.target + '_sorted_list_'+ i.split(' ').join('_');
    // console.log(`This is the sortd_list name thing: ${name}`); *******************aaaaaaaaaaaaaaaaaaahhhhhhhhhhhhhhhhhhhh
...
ExpressionBar.prototype.refresh = function(){
  ...
  if(this.opt.plot === "Bar"){
    this.x = this.renderObject.ScaleRangeX();  
  } else {
...
ExpressionBar.prototype.renderGeneTitles = function(i){  
  ...
  if(renderedTextWidth > blockWidth){
    ...
    if(renderedTextWidth > this.opt.headerOffset){      
      this.opt.headerOffset = renderedTextWidth;      
    }
  }
  ...
  if(renderedTextHeight   > blockWidth  ){
    ...
    geneText.attr('dy', '0.75em');
  }
  
   geneText.style('font-size', fonstSize  + 'px') ;
...
ExpressionBar.prototype.renderChartScale = function(){  
  var scaleContent = `
  <div id="font_size_slidebar" class="size_scales"><h3>Font Size</h3></div>
  <div id="bar_size_slidebar" class="size_scales"><h3>Row Height</h3></div>
  `;
  ...
  chartScales.find('#font_size_slidebar').slider({
    ...
    change: function(event) {      
      ...
      eb.setupSVG();      
      eb.refreshSVG();                        
      // $(`#bar_expression_viewer_options`).css('font-size', value);
      // $(`#bar_expression_viewer_options button`).css('font-size', value);
      // $(`#bar_expression_viewer_options select`).css('font-size', value);
    }
  });
  
  chartScales.find('#bar_size_slidebar').slider({
    min: 10,
    max: 30,
    value: this.opt.barHeight,
    change: function(event) {
      var value = $(this).slider("value"); 

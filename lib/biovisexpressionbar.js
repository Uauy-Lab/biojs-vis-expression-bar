...
ExpressionBar.prototype.setupSVG = function(){
  ...
  style( 'font-size', this.opt.fonstSize + 'px');

  this.chartHead = d3.select('#'+this.chartSVGidHead).
  attr('width', this.opt.width).
  style('font-family', self.opt.fontFamily).
  style( 'font-size', this.opt.barHeight + 'px');
...
ExpressionBar.prototype.setupContainer = function(){
  ...
  <label for="${this.opt.target}_property">Expression unit: </label><select id="${this.opt.target}_property"></Select>
  <input type="checkbox" id="${this.opt.target}_log2"><label for="${this.opt.target}_log2">log2</label></input>
  <button type="button" id="${this.opt.target}_save">Save as SVG</button>
  <button type="button" id="${this.opt.target}_save_png">Save as PNG</button>
  <button type="button" id="${this.opt.target}_save_data">Save data</button>
  <button type="button" id="${this.opt.target}_restore_defaults">Restore Defaults</button>
  <span id="${this.opt.target}_homSpan"><input id="${this.opt.target}_showHomoeologues" type="checkbox"name="showHomoeologues" value="show"><label for="${this.opt.target}_showHomoeologues">Homoeologues</label></input> </span>
  <div id="${this.chartScale}"></div>
  </div>  
  <br/><svg id="${this.chartSVGidHead}" ></svg>
  <div id="${this.sortDivId}" ></div>
  <div style="overflow-y: auto;"><svg id="${this.chartSVGid}"></svg></div>
  <br/><svg id="${this.chartSVGidFoot}"></svg>    
  `
...
// Setting the numeric values based on the window size
ExpressionBar.prototype.setDefaultOptions = function(){
  this.opt = {
   ...
   highlight: null, 
   groupBy: 'groups', 
   groupBarWidth: 20,   // This is a constant
   colorFactor: 'renderGroup', 
   headerOffset: ($(window).height() * 0.017),
   showHomoeologues: false,
   plot:'Bar',
   fonstSize: ($(window).height() * 0.017),
   titleGroupSpace: 1
  }; 
}
...
ExpressionBar.prototype.refreshTitles = function(){
  ...
  .attr('y', function(d,i){
    ...
    return (((pos ) * titleGroupSpace * barHeight));
  });
  ...
  .attr('y', function(d,i){
   ...
   return (((pos) * titleGroupSpace * barHeight)) + 10;
 });
...
ExpressionBar.prototype.renderFactorTitles = function(){
  ...
  // Setting the size of the header offset as the longest factor title
...
ExpressionBar.prototype.highlightRow = function(target){
  ...
  var index = 0;
  if(this.opt.titleGroupSpace != 0)
    index = Math.floor(mouse[1]/(this.opt.barHeight * this.opt.titleGroupSpace));
  else
    index = Math.floor(mouse[1]/this.opt.barHeight);
  console.log(`This is the index ${Math.floor(index)}`);
  if(index == 0){
    index = 1;
  }
  ...
  var pos = index*this.opt.barHeight*this.opt.titleGroupSpace;
  console.log(`This is the pos ${pos} and bar height: ${this.opt.barHeight} and title group: ${this.opt.titleGroupSpace}`);
...
ExpressionBar.prototype.render = function() {  
  ...
  var x=this.x;
  if(data[0]){ 
    this.totalHeight = barHeight * (data[0].length + 2 ) * titleGroupSpace;    
  }else{
...
ExpressionBar.prototype.renderChartScale = function(){  
  var scaleContent = `
  <div id="group_title_horizontal_scale" class="size_scales"><h3>Group Title Size</h3></div>
  <div id="group_title_vartical_scale" class="size_scales"><h3>Group Vertical Spacing</h3></div>
  `;
  var chartScales = $(`#${this.chartScale}`);
  ...
  chartScales.find('#group_title_horizontal_scale').slider({
    min: 8,
    max: 30,
    value: eb.opt.fonstSize,
    change: function(event) {
      var value = $(this).slider("value");
      eb.opt.fonstSize = value;
      eb.setupSVG();
      eb.refreshSVG();
    }
  });
  ...
  var barHeight = this.opt.barHeight;
  chartScales.find('#group_title_vartical_scale').slider({
    min: 1,
    max: 5,
    value: eb.opt.titleGroupSpace,
    change: function(event) {
      var value = $(this).slider("value"); 
      eb.opt.titleGroupSpace = value;        
      console.log(`title gouroup space: ${eb.opt.titleGroupSpace}`);
      eb.refreshSVG();      
    }
  });      
}

ExpressionBar.prototype.resizeChart =  function(){  
  var chart = "bar_expression_viewer"
  var bev = $('#' + chart);
  var headSVG = $('#' + chart + '_header_svg');
  var sortDiv = $('#' + chart + '_sort_div');
  var chartSVG = $('#' + chart + '_chart_svg');
  var footerSVG = $('#' + chart + '_footer_svg');
  var parentWidth = $("#bar_expression_viewer").parent().width();  
  var parentHeight = $(window).height();


  bev.css('width', parentWidth);
  headSVG.css('width', parentWidth);  
  sortDiv.css('width', parentWidth);
  chartSVG.css('width', parentWidth);  
  footerSVG.css('width', parentWidth);  
  eb.opt.width = parentWidth;            
  var newBarHeight = (parentHeight * 0.015);


  // SOme important part
  if(newBarHeight > 15){
    eb.opt.barHeight = (parentHeight * 0.017);
  }    

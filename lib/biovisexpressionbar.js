...
ExpressionBar.prototype.setupContainer = function(){
  ...
  var parentWidth = $(`#${this.opt.target}`).parent().width() - 50; // 50 is the scrollbar width
  if(parentWidth < 900){
    this.opt.width = 1500;
  } else {
  ...
  // console.log('available height: ' + this.opt.height);  

  this._container.height(this.opt.height);

  this._container.append(containerContent);

  // console.log(`optionHeight: ${$(`#${this.opt.target}_options`).height()}`);
  // console.log(`headerHeight: ${$(`#${this.chartSVGidHeadDiv}`).height()}`);
  // console.log(`sortHeight: ${$(`#${this.sortDivId}`).height()}`);    


  var chartHeight = (this._container.height()) - ($(`#${this.chartSVGidHeadDiv}`).height()) - ($(`#${this.opt.target}_options`).height()) - 100;
...
ExpressionBar.prototype.resizeChart =  function(){    
  ...
  var parentWidth = $(`#${this.opt.target}`).parent().width() - 20; // 50 is the scrollbar width
  if(parentWidth < 900){
    this.opt.width = 1500;
  } else {
    this.opt.width = parentWidth;  

...
TernaryPlot.prototype.renderGlobalScale = function(){  
  ...
  // Move the position of the plot when scrolling

    $('#plotContainer').scroll(function(){
	  
      var scrollValue = $('#plotContainer').scrollTop();    	
	  // Amount of scroll
	  var amountOfScroll = scrollValue - initialScrollValue;

      d3.select('#ternaryPlotGroup').attr(`transform`, `translate(${labelWidth},${-20 + scrollValue})`);
...
TernaryPlot.prototype._renderTooltip = function(){
  ...
  this.tooltipBox = svg.append('rect').attr('id', 'toolTipRect');
  this.tooltip = svg.append('text').attr('id', 'toolTipText');
...
TernaryPlot.prototype._showTooltip = function(mouseovertext, evt){
  ...
  x = svgPosition.left + this.opt.labelWidth + 11;
  y = svgPosition.top + 10 + scrollValue;  

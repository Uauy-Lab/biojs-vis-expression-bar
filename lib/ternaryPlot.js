  ...
  for (var j = 0; j < Object.keys(data.expression_bias).length; j++) {
    ...
    if(j === (Object.keys(data.expression_bias).length-1)){
      blockDiv.append('text')
        ...
        .attr('width', 50)
        .attr('height', blockSize)
        .attr('x', blockSize + labelWidth + 80)
        .attr('y', headerOffset - blockSize + 15);
      ...
      blockDiv.append('text')
        ...
        .attr('width', 50)
        .attr('height', blockSize)
        .attr('x', (blockSize * numOfBlocks) + labelWidth + 100)
        .attr('y', headerOffset - blockSize + 15);
    }
...
TernaryPlot.prototype._indicateExpressionBias = function(expressionBias){
  ...
  var self = this;
  var tooltipText = `Average distance from centroid: ${(expressionBias/100).toFixed(2)}`;   // Tooltip text

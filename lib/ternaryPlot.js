...
TernaryPlot.prototype._drawBlockDivisions = function(){
  ...
  var blockContainer = d3.select(`#${chartHeaderId}`).append('g').attr('id', 'expression_bias');

  blockContainer.append('text')
    .text('Expression Bias')
    .attr('width', 100)
    .attr('height', 20)
    .attr('x', labelWidth)
    .attr('y', headerOffset - 50);

  for (var i = 0; i < 10; i++) {
    blockContainer.append('rect')
      .attr('width', 20)
      .attr('height', 20)
      .style('stroke-width', 2)
      .style('stroke', 'black')
      .style('fill', 'white')
      .attr('x', (i*20) + labelWidth + 110)
      .attr('y', (headerOffset - 65));

    // Adding the ticks for blocks
    blockContainer.append('text')
      .style('font-size', 10)
      // .text((i * 10)+10)
      .text((i * 10))
...
TernaryPlot.prototype._calculateExpressionBias = function(data, centroid){
  ...
  // console.log(`Sum of distances: ${sumOfDistances}\nNumber of points: ${data.length}\nExpression Bias: ${expressionBias}`);

  var nChild = (Math.floor(expressionBias /  10) * 2) + 2;  

  d3.select(`#expression_bias rect:nth-child(${nChild})`).style('fill', 'red')    
...
TernaryPlot.prototype._calculateplotSize = function(){  
  ...
  var plotContainerHeight = $(`#${plotContainerId}`).height() - 200; //200 is the margin from the top and bottom  
  var plotContainerWidth = $(`#${plotContainerId}`).width() - labelWidth;
  

  // console.log(`terWidth: ${plotContainerWidth}\nterHeight: ${plotContainerHeight}`);
  // console.log(`chartConHeight: ${$(`#${this.parent.chartSVGid}`).height()}
  //   \nchartConWidth: ${$(`#${this.parent.chartSVGid}`).width()}
  //   \nlabelWidth: ${this.parent.opt.labelWidth}
  //   \nchartSpaceWidth: ${$(`#${this.parent.chartSVGid}`).width() - this.parent.opt.labelWidth}
  //   \nchartSpaceWidth: ${($(`#${this.parent.chartSVGid}`).width() - this.parent.opt.labelWidth)/$(`#${this.parent.chartSVGid}`).width() * 100}%
  //   `);

  if(plotContainerHeight < plotContainerWidth){
    console.log(`Using height: \nh: ${plotContainerHeight}\nw:${plotContainerWidth}
      \nwhole width: ${ $(`#${plotContainerId}`).width() }\nlabelWidth: ${labelWidth}`);
    h = plotContainerHeight - 150;
    w = Math.sqrt((4/3) * (h*h));
  } else {
    console.log(`Using Width: \nh: ${plotContainerWidth}\nw:${plotContainerWidth}
      \nwhole width: ${ $(`#${plotContainerId}`).width() }\nlabelWidth: ${labelWidth}`);

...
TernaryPlot.prototype._drawBlockDivisions = function(){
  ...
  //       .attr('y', (headerOffset - (blockSize*3) + 15)+(j*blockSize*2));
  //   }     

  // }

  blockContainer.append('text').text('Homoeolog Expression Bias').attr('x', labelWidth).attr('y', 20);
...
TernaryPlot.prototype._indicateExpressionBias = function(expressionBias){
  ...
  for(var i=0; i<numOfBlockRows; i++){
    ...
    // Tooltip text
    tooltipText[i] = `Average distance from centroid: ${(expressionBias/100).toFixed(2)}`;
    tooltipText[i] += `\nHomoeolog Expression bias decile: ${index}`;
    tooltipText[i] += `\nHomoeolog  Expression bias range: ${expBiasValues[index-1]} - ${expBiasValues[index]}`;    

    d3.select(`#block-group-${i} rect:nth-child(${index})`).style('fill', expColorCode[Object.keys(expColorCode)[i]][index]);

    // $(`#block-group-${i}`).on('mouseenter', function(){            

    //   var centroid = d3.select('#centroid');
    //   self._indicateCircle(toolTipTextCentroid, centroid);
    //   self._showExpBiasTooltip(tooltipText[i], this);
    //   centroid.attr('display', 'block');
    //   self._renderLinesToCentroid();

    // });

    $(`#block-group-0`).on('mouseenter', function(){                  

      var centroid = d3.select('#centroid');
      self._indicateCircle(toolTipTextCentroid, centroid);
      self._showExpBiasTooltip(tooltipText[0], this);
      centroid.attr('display', 'block');
      self._renderLinesToCentroid();

    });
    $(`#block-group-1`).on('mouseenter', function(){                  

      var centroid = d3.select('#centroid');
      self._indicateCircle(toolTipTextCentroid, centroid);
      self._showExpBiasTooltip(tooltipText[1], this);
      centroid.attr('display', 'block');
      self._renderLinesToCentroid();
...
TernaryPlot.prototype._showExpBiasTooltip = function(mouseovertext, evt){
  ...
  if(match){
    ...
    for(var t=0; t<stringBroken.length; t++){

      tooltip.text(stringBroken[t]);
      tooltip.attr('y', y - (t * 12));
      if(t===(stringBroken.length-1)){
        break;
      }      
      var clonedNode = tooltip.node().cloneNode(true);
      clonedNode.setAttribute("class", "tempTextElement");
      $("#expression-bias").get(0).appendChild(clonedNode);
...
TernaryPlot.prototype._indicateCircle = function(tooltipText, circle){  
  ...
  tooltip_order.forEach(function(element, index){
    ...
    if(thisCircleData[0].get('id') === 'centroid'){
      ...
      tooltipText += `${element} : ${thisCircleData[0].get(element).toFixed(2)}%  Exp Val: ${thisCircleData[0].get(`expval${element}`).toFixed(2)}`;
    }    
  ...
  this.parent.showTooltip(tooltipText, circle.node());

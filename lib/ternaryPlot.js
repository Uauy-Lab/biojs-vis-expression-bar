...
TernaryPlot.prototype.renderGlobalScale = function(){
  ...
  opt.axis_ticks.forEach(function(v) {     
    ...
    // Draw the axis    
    self._drawAxis(coordsArr[0], coordsArr[1], opt.ternColors.right);
    self._drawAxis(coordsArr[1], coordsArr[2], opt.ternColors.left);
    self._drawAxis(coordsArr[2], coordsArr[3], opt.ternColors.bottom);
            
    // Tick labels
    self._drawTicks(coordsArr[0], 60, 'end', -opt.tickLabelMargin, v, opt.ternColors.right);
    self._drawTicks(coordsArr[1], -60, 'end', -opt.tickLabelMargin, 100-v, opt.ternColors.left);
  ...
  this._drawLeftArrows(30, 30, opt.ternColors.right, leftArrowDistance);  
  this._drawRightArrows(30, 30, opt.ternColors.left, rightArrowDistance);  
  this._drawBottomArrows(90, 30, opt.ternColors.bottom, bottomArrowDistance);
...
TernaryPlot.prototype.renderGeneBar = function(i){ 
  ...
  // Loading the data
  this._loadData(d, function(d){ return [d.a, d.b, d.c]}, 'label');
...
TernaryPlot.prototype._loadData = function(data, accessor, bindBy){  //bind by is the dataset property used as an id for the join
    ...
    var circles = svg.selectAll("circle")
      .data( data.map( function(d){ return self._coord(accessor(d)); }), function(d,i){
        if(bindBy){

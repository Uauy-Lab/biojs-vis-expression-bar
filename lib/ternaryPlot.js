...
TernaryPlot.prototype.renderGlobalScale = function(){  
  ...
  var plotContainerHeight = $("#plotContainer").height() - 200; //200 is the margin from the top and bottom
  var plotContainerWidth = $("#plotContainer").width() - labelWidth;
        ...
        circles
          .on('mouseenter', function(){
      			// This variable is used to determine the scroll amount to make the tooltip popup move
      		 	initialScrollValue = $('#plotContainer').scrollTop();
            ...
            if(circleId === 'centroid'){
              self._indicateCircle('Centroid: ', circle)
            } else {
...
TernaryPlot.prototype._indicateExpressionBias = function(expressionBias){
  ...
  for(var i=0; i<numOfBlockRows; i++){
    ...
    // Tooltip text
    tooltipText = `Average distance from centroid: ${(expressionBias/100).toFixed(2)}`;
    tooltipText += `\nHomoeolog Expression bias decile: ${index}`
    tooltipText += `\nHomoeolog  Expression bias range: ${(expressionBias/100).toFixed(2)}`
    ...
    $(`#block-group-${i}`).on('mouseenter', function(){
      ...
      self._showExpBiasTooltip(tooltipText, this);
...
TernaryPlot.prototype._showExpBiasTooltip = function(mouseovertext, evt){
  ...
  if(match){
    ...
    for(var t=0; t<stringBroken.length; t++){
      ...
      var x = tooltip.node().cloneNode(true);
      x.setAttribute("class", "tempTextElement");
      $("#expression-bias").get(0).appendChild(x);

    }


    stringBroken.forEach(function(element, index){      
    ...
    textBox.height *= stringBroken.length;    

...
TernaryPlot.prototype._drawBlockDivisions = function(){
  ...
  // If data doesn't contain expression bias key don't execute the rest
  if(typeof data.expression_bias === "undefined"){    
    return
  }  

  var blockContainer = svg.append('g');

  // blockContainer.append('text').text('Azhurnaya').attr('width', 100).attr('height', 20).attr('x', 400).attr('y', 20);
  // blockContainer.append('text').text('Chinese Spring').attr('width', 100).attr('height', 20).attr('x', 400).attr('y', 60);
  for (var j = 0; j < Object.keys(data.expression_bias).length; j++) {
    blockContainer.append('text').text(Object.keys(data.expression_bias)[j]).attr('width', 100).attr('height', 20).attr('x', 400).attr('y', (j*40)+20);
    for (var i = 0; i < 10; i++) {
      ...
      if(j === (Object.keys(data.expression_bias).length - 1)){  // Adding the ticks for blocks
        blockContainer.append('text')
          .style('font-size', 10)
          // .text((i * 10)+10)
          .text(((i / 10)+0.1).toFixed(1))
          .attr('width', 20)
          .attr('height', 20)                  
          .attr('x', (i*20)+520)
          .attr('y', (j*40)+40);          

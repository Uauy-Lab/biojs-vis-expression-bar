...
TernaryPlot.prototype.renderGeneBar = function(i){ 
  ...
  for (var i=0; i<3; i++) {
    for (var key in this.parent.data.tern){
      if (data[i][0].gene === this.parent.data.tern[key]){
        dataHash[key] = data[i][0].gene
      }
    }

    dataHash.renderIndex = data[i][0].renderIndex;
    dataHash.value = data[i][0].value;
...
TernaryPlot.prototype._loadData = function(data, accessor, bindBy){  //bind by is the dataset property used as an id for the join
    ...
    var circles = svg.selectAll("circle")
      .data( data.map( function(d){ return self._coord(accessor(d)); }));

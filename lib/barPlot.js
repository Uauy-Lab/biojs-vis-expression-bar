...
BarPlot.prototype.renderScales = function(i){
  ...
  var xAxisGroup = this.parent.svgFootContainer.append("g").call(xAxis).attr('class', 'x axis').attr('font-size', (this.parent.opt.fontSize/1.4));
...
BarPlot.prototype._responseToScroll = function(){
  ...
  $(`#${plotContainer}`).scroll(function(){   
    ...
    if (lastScrollLeft != documentScrollLeft){      
      ...
      d3.select(`#${chartSVGidFoot}`).style(`position`, 'relative');
      d3.select(`#${chartSVGidFoot}`).style(`left`, -lastScrollLeft);
